...
def updateHand(hand, word):
    ...
    letterFq = getFrequencyDict(word)

    for letter in letterFq.keys():
        updatedHand[letter] -= letterFq[letter];
...
def isValidWord(word, hand, wordList):
    ...
    for letter in word:
        if copyHand.get(letter) == 0 or letter not in copyHand.keys():
            return False
        copyHand[letter] -= 1
...
def calculateHandlen(hand):
    ...
    """
    length = 0
    for letterCount in hand.values():
        length += letterCount
